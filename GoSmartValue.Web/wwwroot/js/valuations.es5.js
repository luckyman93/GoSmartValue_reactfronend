...
var ValuationRequest = function ValuationRequest() {
    var vars = {
...
function formatCurrency(amount) {
    ...
    return (neg ? "-BWP " : 'BWP ') + parseFloat(amount, 10).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,").toString();
}
    ...
    $.get({
        ...
        success: function success(result) {
            ...
            $.each(result, function () {
                var locality = this;
                $('#LocalityId').append('<option value="' + locality.id + '" data-tokens="' + locality.name + '">' + locality.name + '</option>');
            });
        }
    });
}
...
function SetStreets(localityId, localityName) {
    ...
    $.get({
        ...
        success: function success(result) {
            $('#street-list').empty();
            $.each(result, function () {
                $('#street-list').append('<option value="' + this.streetName + '" data-tokens="' + this.streetName + '">' + this.streetName + '</option>');
            });
        }
    });
}
...
$(document).ready(function () {
    ...
    var map,
        infowindow,
        mapOptions = { zoom: 8, center: { lat: -24.6282, long: 25.9231 } };
    console.log("Document has loaded and ready...");

    document.getElementById("ownership").checked ? $("#ownershipQuestions").show() : $("#ownershipQuestions").hide();
    ...
    for (var i = new Date().getFullYear(); i > 1970; i--) {
        $("#PurchaseYear").append('<option value="' + i + '" data-tokens="' + i + '">' + i + '</option>');
    } $("#ownership").on("change", function () {
        document.getElementById("ownership").checked ? $('#ownershipQuestions').show() : $('#ownershipQuestions').hide();
    });

    $('#street-list').on('click', function () {
        console.log('street clicked..' + $('#street-list').val());
        $('#StreetName').val($('#street-list').val());
    });

    $('#GetValuation').on('click', function () {
    ...
    $("#GetFullReport").on("click", function () {
        var valuationReference = $("#ValuationReference").Text;
